...
function AnimeTitle({small = false, mobileScreen = false, children}) {
  ...
  return (
    <div className='h-auto'>
      {!small && <h1 className="hidden md:block text-brightGreen text-2xl text-center md:text-4xl md:text-left font-bold 2xl:text-6xl">{children}</h1>}
      {small && <h5 className="hidden md:block text-darkGreen text-md text-center md:text-md md:text-left font-medium 2xl:text-4xl">{children}</h5>}
    </div>
  );
  else 
  return (
